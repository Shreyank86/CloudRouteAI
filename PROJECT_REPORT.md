...
NS-3's `FlowMonitor` was heavily customized in C++ to export runtime statistics every 2 seconds. The telemetry captures `queue_utilization`, `delay_ms`, `throughput_mbps`, and `packet_loss_rate`. 

**Why:** Evaluating the network every 2 seconds provided a fast enough feedback loop to react to traffic spikes, while avoiding computational overload inside the simulation engine.
...
*   **Trigger:** Link 4->5 manually brought down via NS-3 API at $t=8s$.
