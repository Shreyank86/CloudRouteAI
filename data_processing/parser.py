...
def parse_flowmon_xml(xml_path: str, scenario_id: str) -> dict:
    ...
    for flow_el in root.findall("FlowStats/Flow"):
        ...
        latency_ms       = (delay_sum / max(rx_packets, 1)) / 1e6
        packet_loss_rate = (tx_packets - rx_packets) / max(tx_packets, 1)
        jitter_ms        = (jitter_sum / max(rx_packets, 1)) / 1e6
...
def main():
    ...
    parser = argparse.ArgumentParser(description="CloudRoute AI M2 Parser")
    parser.add_argument("--scenario", type=str, default="normal", help="Scenario tag for the JSON output")
    args = parser.parse_args()
    
    scenario_id = args.scenario
    xml_path = os.path.join(RAW_DIR, "flow.xml")
    json_path = os.path.join(PROCESSED_DIR, "metrics.json")
    ...
    if os.path.exists(json_path):
        try:
            ...
            # Ensure it has the correct schema
            if isinstance(existing_data, dict) and "runs" in existing_data:
                existing_data["runs"].append(new_run)
                # Overwrite scenario_id in case it was a new run from a different scenario
                existing_data["scenario_id"] = scenario_id
            else:
                existing_data = {
                    "version": "1.0",
                    "scenario_id": scenario_id,
    ...
    print(f"[{scenario_id.upper()}] Successfully processed unified metrics to {json_path}")
